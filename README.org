...
- Currently it's deployed on a Debian system, but it's rather flexible and contains a full GNU Guix System configuration as well.
- Install Guix and run a ~guix pull~ before doing importing the package manifest.
- The configuration itself is placed in ~,~/git/emacs-stuff/configs~ and ~,~/git/emacs-stuff/configs-root~, from where the individual modules can be symlinked into place using [[https://www.gnu.org/software/stow/][GNU Stow]].
- Make sure to tangle this file prior to linking the configs.
...
#+BEGIN_SRC sh :tangle "~/git/emacs-stuff/configs/bin/.bin/scr"
  #!/bin/sh
...
#+BEGIN_SRC sh :tangle "~/git/emacs-stuff/configs/bin/.bin/scr-area"
  #!/bin/sh
...
#+BEGIN_SRC sh :tangle "~/git/emacs-stuff/configs/bin/.bin/chromium-incognito"
  #!/bin/sh
  chromium --incognito
#+END_SRC
...
Here is the setup for channels, only read addition here is ~nonguix~.
...
** .bin/guix-run
#+BEGIN_SRC sh :tangle "~/git/emacs-stuff/configs/guix-run/.bin/guix-run"
  #!/bin/sh
...
#+BEGIN_SRC python :tangle "~/git/emacs-stuff/configs/i3/.config/i3/i3status_wrapper.py"
  #!/usr/bin/env python3
  import json
  import subprocess
  import sys
...
#+BEGIN_SRC fundamental :tangle "~/git/emacs-stuff/configs/rofi/.config/rofi/nord.rasi"
,* {
    font:   "Fira Code 10";
...
** .startx
#+BEGIN_SRC sh :tangle "~/git/emacs-stuff/configs/startx/.startx"
  #!/bin/sh
...
Let's add out system-level packages now; these are *only* the most necessary of packages.
#+BEGIN_SRC scheme :tangle "~/git/emacs-stuff/configs-root/guix/etc/config.scm"
  (packages
   (append
