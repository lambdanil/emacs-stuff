...
I decrease the time to render a frame as much as possible (2ms in this case), this significantly helps eliminate input lag. This value needs to be increased if stuttering becomes an issue.
#+begin_src conf-space :tangle "~/git/emacs-stuff/configs/sway/.config/sway/config"
  output HDMI-A-2 max_render_time 2
...
#+BEGIN_SRC conf-space :tangle "~/git/emacs-stuff/configs/sway/.config/sway/config"
  exec --no-startup-id swayidle timeout 480 "$lock"
#+END_SRC
