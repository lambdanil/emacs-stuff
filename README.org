...
This makes sway start up automatically when logging into ~tty1~, eliminating the need for a login manager.
#+BEGIN_SRC bash :tangle "~/git/emacs-stuff/configs/bash/.bash_profile"
  # if [[ "$(tty)" == "/dev/tty1" ]]
  # then
  #     exec sway
...
~Sway~ is one of the window managers I have a full setup for. While I do usually prefer to use a DE or EXWM, I do have i3 here for completeness.
...
#+BEGIN_SRC conf-space :tangle "~/git/emacs-stuff/configs/sway/.config/sway/config"
  set $mod Mod4
  set $HOME /home/nil
  set $term emacsclient -c -e "(vterm)"
...
#+BEGIN_SRC conf-space :tangle "~/git/emacs-stuff/configs/sway/.config/sway/config"
  ...
  bindsym $mod+d exec --no-startup-id wofi --theme ~/.config/wofi/nord.rasi --show run
  bindsym $mod+b exec --no-startup-id wofi --theme ~/.config/wofi/nord.rasi --show window
...
Add the Guix development environment for Geiser.
#+BEGIN_SRC emacs-lisp :tangle "~/git/emacs-stuff/configs/emacs/.emacs"
