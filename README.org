...
[[https://github.com/erpalma/throttled][throttled]] is a small Python utility, used to fix throttling issues affecting a few certain laptop models with a particular Intel CPU. Since it isn't packaged for Guix, I use a bit of a workaround to make it work.

** opt/throttled/start.sh
#+begin_src sh :tangle ~/git/emacs-stuff/configs-root/throttled/opt/throttled/start.sh :shebang "#!/bin/sh"
...
** conf/throttled.conf
#+begin_src conf :tangle ~/git/emacs-stuff/configs/throttled/conf/throttled.conf
  [GENERAL]
...
#+BEGIN_SRC conf-space :tangle "~/git/emacs-stuff/configs/sway/.config/sway/config"
  ...
  exec --no-startup-id gsettings set org.gnome.desktop.interface gtk-theme Dracula
...
** etc/my-sudoers
#+begin_src conf :tangle ~/git/emacs-stuff/configs-root/sudoers/etc/sudoers-my
  root ALL=(ALL) ALL
  %wheel ALL=(ALL) ALL
  %wheel ALL=(ALL) NOPASSWD:/opt/throttled/start.sh
#+end_src
